...
- **Release 构建显示乱码**：修复了因 R8 混淆导致特定包下类名/属性名被混淆，进而引发特定页面选项显示乱码的问题。
- **工作区临时文件展示异常**：修复了 Agent 使用 `workspace_shell` 修改文件时，UI 错误地将已经不存在的临时文件展示为变更文件的问题。

### 新增功能
- **工作区目录导入与智能过滤**：
  - 支持一键导入整个文件夹到工作区，并提供导入进度显示 UI。
  - 内置 `.gitignore` 递归解析功能（支持否定规则及锚定），并支持自定义排除模式配置。
- **工作区目录同步回传**：
  - 支持将工作区文件同步回原始 SAF 目录。
  - 提供“快速（仅尺寸比对）”与“完整（CRC32 内容校验）”两种差异检查模式。
  - 导入完成后自动生成初始快照以优化后续比对，UI 新增扫描→预览→执行的状态机引导弹窗。
- **工作区变更展示与交互增强**：
  - 增强 `workspace_shell` 变更文件检测，变更列表会自动附加到消息元数据中。
  - 新增路径显示切换开关，支持在“纯文件名”与“完整相对路径”间无缝切换（共享展开/折叠状态）。
  - **智能裁剪**：若所有变更文件同属一个项目目录，自动省略项目名前缀，保持界面简洁。
  - **UI 适配**：全路径模式下改为单列展示并支持横向滚动，避免长路径折行；文件标签支持长按复制当前路径到剪贴板并伴随 Toast 提示。
- **图片压缩**：发送图片给 AI 前自动压缩过大尺寸图片。可在「偏好设置 → 实验性功能 → 图片压缩」中开关及调整最大边长（默认 2048px，聊天预览保持原图不变）。
- **思维链 (CoT) 增强**：
